...
export class SpellBuilder extends BuilderBase {
	...
	__$getSourcesGenerated (cb, fnsDoRefreshSources) {
		...
		const getBtnAdd = () => {
			const $btn = $(`<button class="ve-btn ve-btn-xs ve-btn-default" title="Generate additional spell sources based on the spell's current sources (for example, Eldritch Knight Guerrier for a Magicien spell).">Generate Additional</button>`)
				.click(async () => {
					try {
